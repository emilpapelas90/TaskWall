<div class="p-4 xl:ml-80">
 <nav class="block backdrop-saturate-200 backdrop-blur-2xl bg-opacity-80 border border-white/80 w-full max-w-full px-4 bg-white text-white rounded-xl transition-all sticky top-4 z-20 py-3 shadow-md shadow-blue-gray-500/5">
 <!-- block w-full max-w-full bg-white text-white shadow-none rounded-xl transition-all px-2 py-1 -->
  <div class="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
    ...
    <div class="flex gap-2">
     <div class="flex items-center text-blue-gray-500 mr-3">
      <x-ri-wallet-3-fill class="w-5 h-5 mr-2 transition duration-75"/>
      <span class="block font-sans text-md antialiased font-semibold leading-relaxed pt-0.5">${{ Auth::user()->balance }}</span>
      <Link href="{{route('dashboard')}}" class="bg-blue-gray-50 ml-2 p-1 rounded-md">
       <x-ri-add-line class="w-4 h-4 text-blue-gray-800 transition duration-75"/>
      </Link>
     </div>

     <div class="flex items-center text-blue-gray-500">
      <x-ri-megaphone-fill class="w-5 h-5 mr-2 transition duration-75"/>
      <span class="block font-sans text-md antialiased font-semibold leading-relaxed pt-0.5">${{ Auth::user()->abalance }}</span>
      <Link href="{{route('funds')}}" class="bg-blue-gray-50 ml-2 p-1 rounded-md">
       <x-ri-add-line class="w-4 h-4 text-blue-gray-800 transition duration-75"/>
      </Link>
     </div>
